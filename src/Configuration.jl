...
- `hub::Bool = $HUB_DEFAULT` Run as a workspace hub that never executes notebook or package code itself: workspaces are served at `/w/<id>/`, their notebooks run in a child server per workspace that the hub relays to, and the hub skips every Pkg touch (registry updates) at startup. The `spacestation` CLI and the desktop app run their launcher this way.
...
const ON_CODE_CHANGE_DEFAULT = "lazy"
...
- `on_code_change::String = "$ON_CODE_CHANGE_DEFAULT"` What happens when cell code changes without an explicit run request (e.g. the notebook file was edited externally)? `"lazy"` (default): the changed cells are only *marked stale*. `"autorun"`: classic Pluto, changed cells and their dependents run immediately. With `"lazy"`, the changed cells and their dependents are only *marked stale* — nobody loses their running state, and you (or an external tool) decide when to run them.
