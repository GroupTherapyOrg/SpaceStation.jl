"""
SpaceStation 🟢🟣🔴 — a workspace for Pluto.jl notebooks, built for humans and agents together.

Start it with:

```julia
julia> SpaceStation.run()
```
...
include("./notebook/OutputCache.jl")
...
# Julia 1.13 is supported: its `@doc` change (JuliaLang/julia#59882) is handled in PlutoRunner's
# run_expression.jl, and CI runs the suite on 1.13 as a required job.
function warn_julia_compat()
    if VERSION > v"1.13.9999"
    ...
    if (get(ENV, "JULIA_PLUTO_SHOW_BANNER", "1") != "0" &&
        ...
        if !isfile(fn)
            @info """
              ...
              Welcome to SpaceStation $(PLUTO_VERSION_STR) 🎈🏝
