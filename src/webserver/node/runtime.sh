...
JULIA=${1:?julia}; APP=${2:?app dir}; BUNDLES=${3:-$HOME/.spacestation/bundles}
...
pick_root() {
    ...
    for cand in "${SPACESTATION_LOCAL_ROOT:-}" /tmp "${SLURM_TMPDIR:-}" "${PBS_JOBFS:-}" "${FLUX_JOB_TMPDIR:-}" "${TMPDIR:-}" /dev/shm; do
        ...
        if [ -r /proc/self/mountinfo ]; then
            t=$(fstype_of "$cand"); is_local_fs "$t" || { say "skipping $cand ($t is not a local filesystem)"; continue; }
        fi
        free=$(df -Pk "$cand" 2>/dev/null | awk 'NR==2 {print $4}'); [ "${free:-0}" -ge "$NEED_KB" ] || { say "skipping $cand (not enough room)"; continue; }
        root="$cand/spacestation-$(id -u)"
        mkdir -m 700 "$root" 2>/dev/null
        # ours, a real directory, closed to others: anything else (somebody made it first) is not used
        if [ -d "$root" ] && [ ! -L "$root" ] && [ -O "$root" ] && chmod 700 "$root" 2>/dev/null; then echo "$root"; return 0; fi
...
JULIA_ROOT=$(cd "$(dirname "$JULIA")/.." && pwd -P) || fail "cannot resolve the julia install"
JULIA_VERSION=$("$JULIA" --startup-file=no -e 'print(VERSION)' 2>/dev/null) || fail "julia does not run"
...
# one builder per node at a time (a local lock: mkdir is atomic on a local filesystem)
LOCK="$ROOT/rt-$KEY.lock"
for i in $(seq 1 600); do
    mkdir "$LOCK" 2>/dev/null && break
    ready && { echo "RUNTIME $RT"; exit 0; }
    # a builder that died leaves its lock: take it over after ten minutes of silence
    [ -n "$(find "$LOCK" -maxdepth 0 -mmin +10 2>/dev/null)" ] && { rmdir "$LOCK" 2>/dev/null; continue; }
    sleep 1
done
...
if [ -f "$BUNDLE" ] && [ -f "$BUNDLE.sha256" ]; then
    ...
    if [ "$(sha "$BUNDLE" | cut -d' ' -f1)" = "$(cut -d' ' -f1 "$BUNDLE.sha256")" ] && tar -xzf "$BUNDLE" -C "$ROOT" && [ -x "$RT/julia/bin/julia" ]; then
        mkdir -p "$RT/home" "$RT/tmp"; touch "$RT/.complete"; echo "RUNTIME $RT"; exit 0
    fi
...
mkdir -p "$RT/depot" "$RT/home" "$RT/tmp" || fail "cannot create $RT"
cp -a "$JULIA_ROOT" "$RT/julia" || fail "could not copy julia"
...
mkdir -p "$RT/app" && (cd "$APP" && for f in Project.toml Manifest.toml src frontend frontend-dist bin sample assets; do [ -e "$f" ] && echo "$f"; done | tar --exclude=node_modules --exclude=.parcel-cache -cf - -T -) | tar -xf - -C "$RT/app" || fail "could not copy the app"
...
' "$RT/depot" >&2 || fail "could not copy the packages"
...
( cd "$RT" && env -u JULIA_LOAD_PATH -u JULIA_PROJECT HOME="$RT/home" TMPDIR="$RT/tmp" JULIA_DEPOT_PATH="$RT/depot:" JULIA_CPU_TARGET="$CPU_TARGET" JULIA_PKG_OFFLINE=true \
    "$RT/julia/bin/julia" --startup-file=no --history-file=no --project="$RT/app" -e 'import SpaceStation' >&2 ) || { rm -rf "$RT"; fail "the app does not load from the local runtime"; }
...
(
    ...
    if tar -C "$ROOT" --exclude="rt-$KEY/.complete" --exclude="rt-$KEY/home" --exclude="rt-$KEY/tmp" -czf "$tmp" "rt-$KEY" 2>/dev/null; then
