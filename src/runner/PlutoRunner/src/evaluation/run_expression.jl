...
function doc_target_name(@nospecialize(e))
    ...
    if e.head === :macro
        # a macro's docs binding is `@foo`, not `foo` (which may be a different, unrelated global)
        name = isempty(e.args) ? nothing : doc_target_name(e.args[1])
        return name === nothing ? nothing : Symbol("@", name)
    elseif e.head === :(=) || e.head === :function || e.head === :(->) ||
       e.head === :where || e.head === :call || e.head === :(::) || e.head === :curly ||
       e.head === :(<:) || e.head === :abstract || e.head === :primitive
